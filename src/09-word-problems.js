...
function createLineOrder(people) {
  const members = people.filter(person => person.hasMembership);
  const nonMembers = people.filter(person => !person.hasMembership);
  return members.concat(nonMembers).map(person => person.name);
}
